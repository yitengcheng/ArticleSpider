...
class CaoliuSpider(scrapy.Spider):
    ...
    def parse(self, response):
        post_urls = response.xpath(
            '//*[@id="ajaxtable"]//tr[@class="tr3 t_one tac"]//td[@class="tal"]/h3/a/@href').extract()
    ...
    def parse_detail(self, response):
        images = response.css(
            'input[type="image"]::attr("data-src")').extract()
        item_loader = ArticleItemLoader(
            item=CaoLiuArticleItem(), response=response)
        item_loader.add_xpath(
            'title', '//*[@id="main"]/div[3]/table/tr[1]/th[2]/table/tr/td/h4/text()')
